...
Citizen.CreateThread(function()
    ...
    if Config.TrackedJobs[ESX.PlayerData.job.name] then
        TriggerServerEvent('prx_duty:DutyCheck', ESX.PlayerData.job.name, true)
    end
end)
...
AddEventHandler('esx:setJob', function(job)
    ...
    if Config.TrackedJobs[job.name] and job.name ~= ESX.PlayerData.job.name then
        TriggerServerEvent('prx_duty:DutyCheck', ESX.PlayerData.job.name, false)
    end
...
if Config.AllowReportInEveryWhere then
    ...
    RegisterCommand('duty', function()
        if not timedOut then
            ...
            timedOut = true
            TriggerServerEvent('prx_duty:ChangeJob')
            SetTimeout(5000, function()
                timedOut = false
            end)
        else
            ESX.ShowNotification(_U('no_spam'))
        end
    end)
...
RegisterNetEvent('prx_duty:returnEmployes')
AddEventHandler('prx_duty:returnEmployes', function(Erows)
    ...
    if not ESXJobs then
        TriggerServerEvent('prx_duty:RequestJobs')
    end
    ...
    ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'duty_time_list' .. bossJobName, elements, function(data, menu)
        ...
        if data.value == 'resetTime' then
            ...
            TriggerServerEvent('prx_duty:ResetTime', employee.identifier, bossJobName)
        end
...
Citizen.CreateThread(function()
    ...
    while true do
        if Config.TrackedJobs[ESX.PlayerData.job.name] then
            TriggerServerEvent('prx_duty:DutyTimeUpdate')
        end
        Wait(60 * 1000)
    end

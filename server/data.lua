...
if SConfig.AutoInsert then
    Citizen.CreateThread(function()
        ...
        for k, v in pairs(trackedJobs) do
            local result = MySQL.Sync.fetchAll('SELECT * FROM job_grades INNER JOIN jobs ON jobs.name = "'..k..'"')
            if #result == 0 then
                local jobData = MySQL.Sync.fetchAll('SELECT * FROM job_grades WHERE job_name = @job_name', {
                ['@job_name'] = k:gsub('off', '')
                })
                ...
                if #jobData > 0 then
                    ...
                    MySQL.Async.execute('INSERT INTO jobs (name, label, whitelisted) VALUES (@name, @label, 1)', {
        ...
        if totalJobsConverted > 0 then
            print('[^3prx_duty^0]: ^2Changed total: '..totalJobsConverted..'.^0')
            print('[^3prx_duty^0]: ^8You SHOULD restart your server now.^0')
        end
    end)
end